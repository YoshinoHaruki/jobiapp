...
const baseurl = process.env.APP_MANIFEST.REACT_APP_BASE_URL;
        ...
        <View style={css.cont_white}>
            ...
            <View style={styles.box}>
                ...
                <View style={styles.txt}>
                            {JSON.parse(postData.belongings || "{}").uniform && <View><Text style={styles.txt}>・私服OK</Text></View>}
                            {JSON.parse(postData.belongings || "{}").shoes && <View><Text style={styles.txt}>・シューズ貸し出し</Text></View>}
                            {JSON.parse(postData.belongings || "{}").pen && <View><Text style={styles.txt}>・筆記用具</Text></View>}
                            {JSON.parse(postData.belongings || "{}").snack && <View><Text style={styles.txt}>・飲み物・軽食</Text></View>}
                </View>
                <View style={styles.border}>
                    <View style={styles.box_check}>
                ...
                </View>
                {
                    ...
                    <View style={{ position: 'initial' }}>
                        ...
                        <View style={styles.txt}>
                            {JSON.parse(postData.treatment || "{}").bike && <View><Text style={styles.txt}>・バイク通勤OK</Text></View>}
                            {JSON.parse(postData.treatment || "{}").car && <View><Text style={styles.txt}>・車通勤OK</Text></View>}
                            {JSON.parse(postData.treatment || "{}").launchTime && <View><Text style={styles.txt}>・まかないつき</Text></View>}
                            {JSON.parse(postData.treatment || "{}").beginner && <View><Text style={styles.txt}>・未経験者OK</Text></View>}
                            {JSON.parse(postData.treatment || "{}").transfee && <View><Text style={styles.txt}>・交通費支給</Text></View>}
                            {JSON.parse(postData.treatment || "{}").bicycle && <View><Text style={styles.txt}>・自転車通勤OK</Text></View>}
                            {JSON.parse(postData.treatment || "{}").atHome && <View><Text style={styles.txt}>・アットホーム</Text></View>}                            
                        </View>
                        <View style={styles.border}>
                            <View style={styles.box_check}>
